...
def ray_casting(player_pos, player_angle, world_map):
    ...
    for ray in range(NUM_RAYS):
        ...
        (depth, offset, texture) = ((depth_v, yv, texture_v) if depth_v
                                    < depth_h else (depth_h, xh,
                                    texture_h))
...
def ray_casting_walls(player, textures):
    ...
    for (ray, casted_values) in enumerate(casted_walls):
        ...
        wall_column = textures[texture].subsurface(offset
                * TEXTURE_SCALE, 0, TEXTURE_SCALE, TEXTURE_HEIGHT)
        wall_column = pygame.transform.scale(wall_column, (SCALE,
                proj_height))
